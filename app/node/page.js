...
const Node = () => {
    ...
    const HandleResponse = (data, id) => {
        ...
        setSearch("")
        setTimeout(() => {handleTransform()},250)
        setLoading(false)
    }
    ...
    return (
        <div className="w-screen h-screen bg-gradient-to-r from-indigo-500">
            <div
                className="absolute inset-0 h-full w-full bg-[#121212] bg-[linear-gradient(to_right,#ce8fff11_1px,transparent_1px),linear-gradient(to_bottom,#ce8fff11_1px,transparent_1px)] bg-[size:24px_24px]"
            > 
            ...
            {show ? <div className="absolute bottom-1/2 left-1/2 transform -translate-x-1/2 xl:w-5/12 md:w-8/12 sm:w-10/12">
                <div id="chat-listeners" className="my-2 w-full  shadow-xl shadow-[#c072fc44]">
            ...
            <div className={`fixed top-2 right-2 h-screen pb-4 w-3/12 bg-transparent transition-transform duration-500 transform drop-shadow-2xl rounded-xl ${isNodeOpen ? '-translate-x-0' : 'translate-x-full hidden'}`}>
                <NodeDetails nodeData={nodeData} isNodeOpen={isNodeOpen} handleNodeOpen={handleNodeOpen} handleSubPromptData={handleSubPromptData} handleLoading={() => {setLoading(true)}}/>
            </div>
            <div className={`fixed top-2 left-2 h-screen pb-6 w-3/12 bg-transparent transition-transform duration-500 transform drop-shadow-2xl rounded-xl ${isNodeOpen ? 'translate-x-0' : '-translate-x-full hidden'}`}>
                <Sidebar/>
            </div>
        </div>
    )
